...
.. note::
   ...
   instead of using ``west``, you can still opt to do so. Inspect the
   `Manifest repository`_ to view the projects included and clone them
   individually or instruct your current repository management tool to do so.
   If you do not clone ``west`` itself then some functionality might not be
   available.
